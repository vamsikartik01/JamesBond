...
export class ControlPanelService {
    ...
    getAllDevices(){
        this.http.get(this.url+"/getdevices/1").subscribe(responseData => {
    ...
    addDevice(name: string, type: string, roomId: string, hubId: string, hubPort: string){
        const mode = "output"
        var url = this.url+"/adddevice/1?name="+name+"&roomId="+roomId+"&hubId="+hubId+"&mode="+mode+"&type="+type+"&hubPort="+hubPort
        console.log(url)
    ...
    editDevice(id: string, name: string, type: string, hubId: string, hubPort: string){
        var url = this.url+"/editdevice/1?id="+id+"&name="+name+"&type="+type+"&hubPort="+hubPort+"&hubId="+hubId
        console.log(url)
    ...
    deleteDevice(id: string){
        var url = this.url+"/deletedevice/1?id="+id
    ...
    setFavorite(id: string, value: boolean){
        var url = this.url+"/setfavorite/1?id="+id+"&value="+value
        console.log(url)
        this.http.put(url, {}).subscribe(
            responseData => {
                console.log(responseData)
    ...
    setValue(id: string, value: string, hubPort: string, hubRefID: string){
        var url = this.url+"/setvalue/1?id="+id+"&value="+value+"&hubPort="+hubPort+"&hubRefId="+hubRefID
        console.log(url)
        this.http.put(url, {}).subscribe(
            responseData => {
                console.log(responseData)
